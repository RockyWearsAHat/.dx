...
async function applyViewerAction(session, actionSpec) {
    ...
    if (action === 'save') {
        ...
        const saved = await saveDocumentSourceByRelativePath(runtime.workspaceRoot, runtime.db, session.document.relativePath, stringifyDocFile(session.document));
...
async function handleTool(id, toolName, args = {}) {
    try {
        ...
        switch (toolName) {
            ...
            case 'save-document': {
                const runtime = await getRuntime(args.workspacePath);
                result = await saveDocumentSourceByRelativePath(runtime.workspaceRoot, runtime.db, String(args.path || ''), String(args.content || ''));
                break;
            }
