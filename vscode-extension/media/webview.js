...
const webview_fsm_mjs_1 = require("./webview-fsm.mjs");
const webview_autocomplete_controller_js_1 = require("./webview-autocomplete-controller.js");
const webview_block_renderer_js_1 = require("./webview-block-renderer.js");
const webview_block_presentation_js_1 = require("./webview-block-presentation.js");
const webview_doc_model_js_1 = require("./webview-doc-model.js");
const webview_edit_controllers_js_1 = require("./webview-edit-controllers.js");
const webview_events_js_1 = require("./webview-events.js");
const webview_state_core_js_1 = require("./webview-state-core.js");
const webview_document_lifecycle_js_1 = require("./webview-document-lifecycle.js");
const webview_ui_state_js_1 = require("./webview-ui-state.js");
const webview_surface_controller_js_1 = require("./webview-surface-controller.js");
...
let docViewState = webview_fsm_mjs_1.DOC_VIEW_STATES.BOOTSTRAPPING;
let docSaveState = webview_fsm_mjs_1.DOC_SAVE_STATES.IDLE;
let lastSavedDoc = '';
...
const docViewMachine = new webview_state_core_js_1.TableDrivenStateMachine('doc-view', webview_fsm_mjs_1.DOC_VIEW_STATES.BOOTSTRAPPING, webview_fsm_mjs_1.DOC_VIEW_TRANSITIONS);
const docSaveMachine = new webview_state_core_js_1.TableDrivenStateMachine('doc-save', webview_fsm_mjs_1.DOC_SAVE_STATES.IDLE, webview_fsm_mjs_1.DOC_SAVE_TRANSITIONS);
const fsmTransitionHistory = new webview_state_core_js_1.TransitionHistory(FSM_HISTORY_LIMIT);
const documentHistory = new webview_state_core_js_1.CallbackUndoRedoController({
...
function ensureBlockRendererController() {
    ...
    blockRendererController = (0, webview_block_renderer_js_1.createBlockRenderer)({
        applyBlockViewPresentation: webview_block_presentation_js_1.applyBlockViewPresentation,
        listItemText: webview_doc_model_js_1.listItemText,
        splitClassNames,
...
function ensureAutocompleteController() {
    ...
    autocompleteController = (0, webview_autocomplete_controller_js_1.createAutocompleteController)({
...
function ensureDocumentLifecycleController() {
    ...
    documentLifecycleController = (0, webview_document_lifecycle_js_1.createDocumentLifecycle)({
        getDocModel: () => docModel,
        setDocModel: (nextDocModel) => {
            docModel = nextDocModel;
        },
        stringifyDoc: webview_doc_model_js_1.stringifyDoc,
        parseDoc: webview_doc_model_js_1.parseDoc,
        ...
        DOC_SAVE_STATES: webview_fsm_mjs_1.DOC_SAVE_STATES,
...
function restoreFsmSnapshot(snapshot) {
    ...
    docViewState = snapshot.docViewState || webview_fsm_mjs_1.DOC_VIEW_STATES.BOOTSTRAPPING;
    docSaveState = snapshot.docSaveState || webview_fsm_mjs_1.DOC_SAVE_STATES.IDLE;
...
function syncVisibleStateFromMachine() {
    ...
    if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.IDLE) {
        clearStatusPersistent();
    }
    else if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.DIRTY) {
        setStatusPersistent('Unsaved changes', 'dirty');
    }
    else if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.SAVING) {
        setStatusPersistent('Saving…', 'saving');
    }
    else if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.SAVED) {
        setStatusPersistent('Saved', 'saved');
    }
    else if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.ERROR) {
...
function isEditModeState(state) {
    return state === webview_fsm_mjs_1.DOC_VIEW_STATES.LOADING_EDIT || state === webview_fsm_mjs_1.DOC_VIEW_STATES.READY_EDIT;
}
function isReadyViewState(state) {
    return state === webview_fsm_mjs_1.DOC_VIEW_STATES.READY_EDIT || state === webview_fsm_mjs_1.DOC_VIEW_STATES.READY_READ;
}
...
function currentDocSourceText() {
    ...
    return (0, webview_doc_model_js_1.stringifyDoc)(docModel);
}
...
function extractCssFromDocumentModel() {
    return (0, webview_doc_model_js_1.extractCssFromDocumentModel)(docModel);
}
function extractStylesheetLinksFromDocumentModel() {
    return (0, webview_doc_model_js_1.extractStylesheetLinksFromDocumentModel)(docModel);
}
...
function publishViewState(effectiveCss) {
    ...
    const sourceText = docModel ? (0, webview_doc_model_js_1.stringifyDoc)(docModel) : '';
...
function collectKnownClasses() {
    ...
    for (const block of docModel.blocks) {
        ...
        const raw = String(block && block.rawSource ? block.rawSource : '');
        const header = raw.split('\n')[0] || '';
        const attrs = (0, webview_doc_model_js_1.parseAttributes)(header.replace(/^::[a-z-]+\s*/i, ''));
...
function renderGhostText(textarea, mirrorEl) {
    if (!mirrorEl || !autocompleteState.suggestions.length)
        return;
...
function findCssBlockIndex() {
    return (0, webview_doc_model_js_1.findCssBlockIndex)(docModel);
}
...
function upsertCssBlock(cssText) {
    ...
    if (cssIndex < 0) {
        ...
        block.rawSource = (0, webview_doc_model_js_1.stringifyBlock)(block);
    ...
    block.language = 'css';
    block.text = text;
    block.rawSource = (0, webview_doc_model_js_1.stringifyBlock)({ ...block, rawSource: '' });
}
function ensureControllers() {
    ensureBlockRendererController();
    ensureAutocompleteController();
    if (!inlineCssController) {
        inlineCssController = new webview_edit_controllers_js_1.InlineCssSurfaceController({
            applyCustomCss,
    ...
    if (!blockSourceController) {
        blockSourceController = new webview_edit_controllers_js_1.BlockSourceController({
            ...
            knownBlockTypes: webview_doc_model_js_1.KNOWN_BLOCK_TYPES,
            parseBlock: webview_doc_model_js_1.parseBlock,
            recordDocumentUndo,
            refreshDocumentCss,
            renderDocument,
            setStatus,
            splitBlockSourceForEditor,
            stringifyBlock: webview_doc_model_js_1.stringifyBlock,
...
function isBlankPageClickTarget(target, pageEl, blocksContainer) {
    if (!target || !pageEl || !blocksContainer)
        return false;
    if (target.closest('.ui-chrome'))
        return false;
    if (target.closest('.meta-rendered') || target.closest('.meta-input'))
        return false;
    if (target.closest('.block-wrap'))
        return false; // Don't close on any click within blocks
    if (target.closest('.block-view') || target.closest('.block-src-wrapper'))
        return false;
    if (target.closest('.autocomplete-menu'))
        return false; // Don't close on autocomplete interactions
...
function ensureUiStateController() {
    ...
    uiStateController = (0, webview_ui_state_js_1.createUiStateController)({
        appearanceStorageKey,
        editModeStorageKey,
        DOC_VIEW_STATES: webview_fsm_mjs_1.DOC_VIEW_STATES,
...
function summarizeBlockTypeCounts() {
    ...
    for (const block of docModel.blocks) {
        ...
        if (type === 'heading')
            ...
            counts.paragraphs += 1;
        else if (type === 'bulleted-list' || type === 'numbered-list' || type === 'checklist')
            counts.lists += 1;
        else if (type === 'code')
            counts.codeBlocks += 1;
        else if (type === 'image')
...
function ensureSurfaceController() {
    ...
    surfaceController = (0, webview_surface_controller_js_1.createSurfaceController)({
        getDocModel: () => docModel,
...
function hideLoadingScreen() {
    ...
    if (docViewState === webview_fsm_mjs_1.DOC_VIEW_STATES.BOOTSTRAPPING) {
...
function armLoadingGuard(preferredEditMode = true) {
    ...
    loadingGuardTimer = setTimeout(() => {
        ...
        if (docViewState === webview_fsm_mjs_1.DOC_VIEW_STATES.BOOTSTRAPPING || docViewState === webview_fsm_mjs_1.DOC_VIEW_STATES.LOAD_ERROR) {
...
function insertImageBlock(imagePath, altText, atIndex) {
    ...
    block.rawSource = (0, webview_doc_model_js_1.stringifyBlock)(block);
    const insertAt = typeof atIndex === 'number' && Number.isFinite(atIndex) && atIndex >= 0
    ...
    function isDraggedImages(event) {
        ...
        if (dt.items && dt.items.length > 0) {
            for (let i = 0; i < dt.items.length; i += 1) {
                if (String(dt.items[i].type || '').startsWith('image/'))
                    return true;
            }
        }
...
function initializeDocument() {
    ...
    try {
        ...
        if (!raw) {
            ...
            docModel = (0, webview_doc_model_js_1.parseDoc)(sourceText);
        }
        renderDocument();
        lastSavedDoc = (0, webview_doc_model_js_1.stringifyDoc)(docModel);
    }
    catch (error) {
    ...
    if (false && blocksContainer) {
        ...
        blocksContainer.addEventListener('focusout', (event) => {
            if (!event.target.classList.contains('block-src'))
                return;
        ...
        blocksContainer.addEventListener('keydown', (event) => {
            if (!event.target.classList.contains('block-src') && !event.target.classList.contains('block-edit-header'))
                return;
        ...
        blocksContainer.addEventListener('click', (event) => {
            const target = getEventElementTarget(event);
            if (!target || !target.classList.contains('block-edit-header'))
                return;
            updateInlineCssAffordance(target);
        });
        blocksContainer.addEventListener('click', (event) => {
            const target = getEventElementTarget(event);
            if (!target || !target.classList.contains('block-src'))
                return;
        ...
        blocksContainer.addEventListener('wheel', (event) => {
            ...
            if (!headerEditor)
                return;
            if (headerEditor.style.display === 'none')
                return;
            event.preventDefault();
            const caret = String(headerEditor.value || '').length;
            headerEditor.focus();
        ...
        blocksContainer.addEventListener('scroll', (event) => {
            ...
            if (mirrorEl)
                mirrorEl.scrollTop = event.target.scrollTop;
        }, true);
    ...
    if (blocksContainer) {
        ensureControllers();
        (0, webview_events_js_1.registerBlockInteractionEvents)({
            blocksContainer,
    ...
    document.addEventListener('mousedown', (event) => {
        ...
        if (!target)
            return;
        if (target.closest('.block-src-wrapper'))
            return;
        if (target.closest('.inline-css-surface'))
            return;
        if (target.closest('.block-view'))
            return;
    ...
    document.addEventListener('change', (event) => {
        ...
        if (!ul || !ul.classList.contains('checklist-wrap'))
            return;
        ...
        if (!Number.isFinite(blockIndex) || !Number.isFinite(itemIndex))
            return;
        const block = docModel && docModel.blocks && docModel.blocks[blockIndex];
        if (!block || block.type !== 'checklist' || !Array.isArray(block.items))
            return;
    ...
    window.addEventListener('message', (event) => {
        ...
        if (msg.type === 'save-complete') {
            ...
            if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.DIRTY) {
                setStatusPersistent('Unsaved changes', 'dirty');
                return;
            }
            transitionDocSaveState('SAVE_COMPLETE');
            lastSaveErrorMessage = '';
            lastSavedDoc = (0, webview_doc_model_js_1.stringifyDoc)(docModel || { metadata: {}, blocks: [] });
            hasDirtyWorkingCopySignal = false;
            setStatusPersistent('Saved', 'saved');
            setTimeout(() => {
                if (docSaveState === webview_fsm_mjs_1.DOC_SAVE_STATES.SAVED) {
