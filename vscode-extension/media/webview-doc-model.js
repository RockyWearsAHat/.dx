...
exports.KNOWN_BLOCK_TYPES = void 0;
...
exports.extractCssFromDocumentModel = extractCssFromDocumentModel;
exports.extractStylesheetLinksFromDocumentModel = extractStylesheetLinksFromDocumentModel;
exports.findCssBlockIndex = findCssBlockIndex;
const doc_pipeline_js_1 = require("./doc-pipeline.js");
...
exports.KNOWN_BLOCK_TYPES = new Set([
...
function parseDoc(source) {
    return { blocks: (0, doc_pipeline_js_1.parseSourceBlocks)(source) };
}
...
function extractCssFromDocumentModel(model) {
...
function extractStylesheetLinksFromDocumentModel(model) {
    if (!model || !Array.isArray(model.blocks)) {
        return [];
    }
    const links = [];
...
function findCssBlockIndex(model) {
    if (!model || !Array.isArray(model.blocks)) {
        return -1;
    }
    for (let i = 0; i < model.blocks.length; i += 1) {
