...
exports.DEFAULT_BLOCK_AUTOCOMPLETE = void 0;
exports.getLineContextFromValue = getLineContextFromValue;
exports.computeAutocompleteSuggestions = computeAutocompleteSuggestions;
exports.computeNextSelectedIndex = computeNextSelectedIndex;
exports.DEFAULT_BLOCK_AUTOCOMPLETE = [
    '::paragraph',
    '::heading level=1',
    '::bulleted-list',
    '::numbered-list',
...
function getLineContextFromValue(value, cursor) {
...
function computeAutocompleteSuggestions(options) {
    ...
    if (headerMatch) {
        ...
        if (valueMatch) {
            ...
            if (key === 'id') {
                ...
                for (const value of knownClasses)
                    candidates.add(value);
            }
            else if (key === 'src') {
                for (const value of knownImageSources)
                    candidates.add(value);
            }
            else if (key === 'type') {
                for (const value of knownBlockTypes)
                    candidates.add(value);
            }
            for (const candidate of candidates) {
                if (!candidate || !candidate.startsWith(typed) || candidate === typed)
                    continue;
                suggestions.push({
                    label: candidate,
                    insertText: candidate,
        ...
        if (keyMatch) {
            ...
            for (const key of keys) {
                if (!key.startsWith(typed) || key === typed)
                    continue;
                suggestions.push({
                    label: `${key}=`,
                    insertText: `${key}=`,
    ...
    if (classMatch) {
        ...
        for (const token of knownClasses) {
            if (!token.startsWith(currentToken) || token === currentToken)
                continue;
            suggestions.push({
                label: token,
                insertText: token,
...
function computeNextSelectedIndex(currentIndex, delta, count) {
