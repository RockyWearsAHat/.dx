...
`dx set` (`--header` to retype; `--replace OLD --with NEW` for the change-sized in-place
edit — a rename pays for the characters that changed, never a retyped block), `dx insert`,
`dx remove`, `dx check`, `dx render --block`,
